...
const LoginScreen = () => {
  ...
  useEffect(() => {
    ...
    const startSession = async () => {
      ...
      try {
        const response = await axios.post(
          "http://35.223.244.137:8000/v1/user/session/start",
          {},
          {
            headers: {
    ...
    const handleAppStateChange = async (nextAppState) => {
      ...
      if (nextAppState === "active") {
        ...
        try {
          const response = await axios.post(
            "http://35.223.244.137:8000/v1/user/session/end",
            {},
            {
              headers: {
                Authorization: `Bearer ${token}`,
  ...
  const handleEmailSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        `http://35.223.244.137:8000/v1/auth/initLogin`,
        {
  ...
  const handleLogin = async (savedEmail = null, savedPassword = null) => {
    ...
    try {
      const response = await axios.post(
        `http://35.223.244.137:8000/v1/auth/login`,
        {
